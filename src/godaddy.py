...
from src.name_checker import EnvType, NameChecker
...
class GoDaddy(NameChecker):
    ...
    def __init__(
        ...
        super().__init__(env_type, names, batch_size, max_retries)
    ...
    def check_batch(self, batch: List[str]) -> List[Dict[str, Union[str, bool]]]:
        json_results = []
        calc_results = []
    ...
    def check(self) -> None:
        if self.limit is not None:
            self.names = self.names[:self.limit]
